...
def main():
    ...
    for chr_n in chrm_list:
        ...
        feature_out_file = "output/50kb_resolution_intrachromosomal/GAT2VEC-input/" + chrm + "/" + chrm + "_na.adjlist"

        os.makedirs(os.path.dirname(feature_out_file), exist_ok=True)
        ...
        for y in range(0, num_bins):
            if (y + 1) not in bin_list:
                continue
            feature_out.write("{}".format(bin_list.index(y + 1) + 1))
            for z in range(0, num_domains):
                if features_matrix[y, z] == 1:
                    feature_out.write(" {}".format(z + 10000))
            feature_out.write("\n")
